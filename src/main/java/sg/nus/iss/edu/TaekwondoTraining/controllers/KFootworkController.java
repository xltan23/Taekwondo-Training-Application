...
@Controller
@RequestMapping(path = "/kyorugi/footwork")
public class KFootworkController {
    ...
    // Link to access user's arms workout page (localhost:8080/kyorugi/footwork/{user})
    @GetMapping("{user}")
    public String getUserFootwork(
    ...
    // Form info post to (localhost:8080/kyorugi/footwork/saveduration)
    // Save workout to temporary workout list 
    // Return to (localhost:8080/kyorugi/footwork/{user})
    ...
    public String postDuration(@RequestBody MultiValueMap<String,String> form, Model model) {
        ...
        return "redirect:/kyorugi/footwork/%s".formatted(userTrim);
    }

    // Form info post to (localhost:8080/kyorugi/footwork/saverepetition)
    // Save workout to temporary workout list 
    // Return to (localhost:8080/kyorugi/footwork/{user})
